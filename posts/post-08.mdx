...
업데이트하기 위한 패턴 및 라이브러리다. 전역에서 사용되는 상태를 위한 중앙 저장소 역할을 하며, 예측 가능한 방식으로만 상태를 업데이트할 수 있도록 규칙을 사용한다.
...
액션은 타입 필드가 있는 자바스크립트 객체다. 애플리케이션에서 발생하는 일을 설명하는 이벤트 역할을 하며,
`type`에는 일반적으로 `domain/event`로 작성된다.
추가 정보가 포함될 수 있는데 보통 `payload` 필드에 넣는 것이 관례다.
...
스토어는 상태가 관리되는 저장소다. 컴포넌트는 스토어를 통해 상태를 전달받게 된다.
...
store를 통해 상태를 전달한다면, dispatch를 통해 액션 타입을 스토어에 전달할 수 있다. 스토어는 전달받은 action으로 상태를 업데이트할 수 있게 된다. dispatch는 이벤트 트리거와 같다. 스토어에 어떤 이벤트가 발생됐는지 알리고 리듀서는 리스너처럼 작동하여 상태를 업데이트 하게 된다.
...
4. 스토어는 구독 중인 컴포넌트에 상태 값이 업데이트 됨을 알리고 컴포넌트는 리렌더링하여 UI를 업데이트한다.
