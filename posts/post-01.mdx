...
date: 2023-07-26
description: DOM이 업데이트될 때마다 render 트리를 재 생성하고 과정을 거치게 된다. 기존의 웹은 DOM을 자주 변경하지 않고 정적인 페이지로 구성했지만, SPA (React, Vue, Angular)가 등장하면서 DOM을 업데이트할 일이 잦아졌다. 좀 더 빠르게 DOM을 업데이트하기 위해 Virtual DOM이 등장했다.
url: virtual-dom
---

DOM이 업데이트될 때마다 render 트리를 재 생성하고 [reflow와 repaint](/브라우저의-렌더링-과정) 과정을 거치게 된다.<br/>
...
Virtual DOM이 하는 것은 DOM fragment를 사용하여 하나로 묶어 적용한 다음 DOM에 던져주는 과정에 불과하다.

### Virtual DOM은 무조건 빠르지 않다.

인터랙션이 많은 앱의 경우 Virtual DOM이 빠를 수 있지만 정적인 페이지의 경우 느릴 수 있다.
