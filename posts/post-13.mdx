...
date: 2023-08-10
url: css-text-ellipsis
thumbnail: /imgs/posts/13/a2f73b47-8796-4801-8515-b285ea693b0a.png
---

css를 사용해서 말 줄임 처리를 하기 위해서는 display 속성 값에 따라 약간의 코드가 추가된다.  
(js를 사용하지 않고 css로 문제를 해결할 수 있다면 성능 측면에서 이점을 가져갈 수 있다)  
...
아래 예제는 `<div>`태그 안에 `<h2>`와 `<span>`태그가 존재한다.
...
`<span>` 태그의 텍스트를 한 줄로 변경하려면 css 속성은 아래와 같다.
